...
- 新增 deck plan → 原生 PPTX 构建器，可生成可编辑文字、形状、图表、表格和讲稿备注。
- 新增真实原生 PPTX 案例，包含重导入后的逐页渲染、联系表、对象检查、编辑性报告和可逆文字对象探针。
...
Skill 本体由 Markdown、Python、JavaScript、JSON 和 HTML 组成。校验脚本只使用 Python 标准库；可选的原生 PPTX 构建器需要 Node.js 和准备好的 `@oai/artifact-tool` workspace。
...
| 模式 | 适用场景 | 主要取舍 |
|---|---|---|
| `native-pptx` | PowerPoint 可编辑性、团队协作、图表、表格、备注、客户交付 | 内置构建器需要准备好的 `@oai/artifact-tool` workspace；导入旧 deck 的修复仍需经验证的原生编辑器 |
...
`examples/case-studies/native-editable/` 包含真实 `.pptx`、6 张重导入后渲染的 PNG、联系表、对象检查日志、编辑性报告、可逆编辑探针和 QA 报告。案例包含原生文字、形状、图表、表格和每页备注，不包含整页图片。

```bash
python3 scripts/check_native_pptx_case.py
```

准备好 `@oai/artifact-tool` workspace 后，可从 deck plan 构建新的原生 PPTX：
